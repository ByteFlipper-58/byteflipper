...
export default {
  ...
  data() {
    return {
      ...
      ENABLE_RESET_CACHE,
      audioPlayer: null,
      isAudioMuted: false, // Changed to false by default
    };
  },
  methods: {
    toggleMenu() {
    ...
    handleClickOutside(event) {
      ...
      if (
        ...
        !this.$el.querySelector('button').contains(event.target)
      ) {
        this.closeMenu();
      }
    },
    ...
    loadSnowingStateFromStorage() {
      if (this.ENABLE_SNOW_TOGGLE) {
        ...
        this.isSnowingEnabled = storedSnowingState === null ? true : storedSnowingState === 'true';
      }
    },
    loadMusicStateFromStorage() {
      if (this.ENABLE_MUSIC) {
        const storedMusicState = localStorage.getItem('isAudioMuted');
        // Changed to match snow behavior - default to unmuted if no stored state
        this.isAudioMuted = storedMusicState === null ? false : storedMusicState === 'true';
      }
    },
  ...
  if (this.audioPlayer) {
    ...
    // Добавляем обработчик первого клика для разрешения воспроизведения
    const startAudio = () => {
      if (!this.isAudioMuted) {
        this.audioPlayer.play().catch(error => {
          console.error('Не удалось воспроизвести аудио после взаимодействия:', error);
        });
      }
